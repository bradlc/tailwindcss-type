...
module.exports = function polyFluidSizing(
  selector,
  properties,
  map,
  ...
  if (rem) {
    pxToRem = (
      ...
      return px.replace(/([0-9.]+)px$/gi, (_, p1) => {
        return `${parseFloat(p1) / base}${isMediaQuery ? 'em' : 'rem'}`
      })
    }
  }
  ...
  for (let i = 0; i < keys.length - 1; i++) {
    let key = keys[i]
    let media = `@media (min-width: ${pxToRem(key, 16, true)})`
  ...
  rules[`@media (min-width: ${pxToRem(keys[keys.length - 1], 16, true)})`] = {
